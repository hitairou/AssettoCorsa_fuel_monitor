...
def layout(window_size):
    ...
    rows = []
    group_headers = []
    current_y = legend_y + LEGEND_H + LEGEND_TO_GROUP_GAP
    ...
    content_bottom_y = energy_row["bar_y"] + energy_row["bar_h"]
    required_h = content_bottom_y + BOTTOM_SAFE_MARGIN + TITLEBAR_EXTRA_H
...
def create(window_id):
    ...
    labels["axis_t_right"] = add_label(
        window_id, "0s", geo["graph_x"] + geo["graph_w"] - 28, geo["graph_y"] + GRAPH_H + 1, 28, 12, 9, "right"
    )
    ...
    for group_header in geo["group_headers"]:
        ...
        labels["sep_" + group_header["key"]] = add_label(
            window_id,
            "──────────────────────────────",
            group_header["x"] + 154,
            group_header["y"],
            max(group_header["w"] - 154, 20),
            GROUP_HEADER_H,
            HEADER_FONT,
            "left",
            (0.45, 0.50, 0.58, 0.85),
...
def update(labels, state):
    ...
    move(labels["axis_top"], PADDING, geo["graph_y"], AXIS_W - 2, 14)
    move(labels["axis_zero"], PADDING, geo["graph_y"] + GRAPH_H / 2 - 7, AXIS_W - 2, 14)
    move(labels["axis_bottom"], PADDING, geo["graph_y"] + GRAPH_H - 14, AXIS_W - 2, 14)
    move(labels["axis_t_left"], geo["graph_x"], geo["graph_y"] + GRAPH_H + 1, 40, 12)
    move(labels["axis_t_right"], geo["graph_x"] + geo["graph_w"] - 28, geo["graph_y"] + GRAPH_H + 1, 28, 12)
    for idx, _legend in enumerate(LEGEND):
        x = PADDING + idx * 82
        move(labels["legend_" + str(idx)], x, geo["legend_y"], 78, 14)
    ...
    for group_header in geo["group_headers"]:
        move(labels["group_" + group_header["key"]], group_header["x"], group_header["y"], 260, GROUP_HEADER_H)
        move(
            labels["sep_" + group_header["key"]],
            group_header["x"] + 154,
            group_header["y"],
            group_header["w"] - 154,
            GROUP_HEADER_H,
...
def _formula_tokens_for_row(row_key, state, compact=False):
    ...
    if row_key == "roll":
        return [
            ("Crr[" + fmt_float(crr, 4) + "]×m[" + fmt_float(mass, 1) + "]×g[" + fmt_float(g, 2) + "]×", "const"),
    ...
    if row_key == "aero":
        return [
            ("0.5×ρ[" + fmt_float(rho, 2) + "]×Cd[" + fmt_float(cd, 3) + "]×A[" + fmt_float(area, 3) + "]×", "const"),
            ("v[" + fmt_float(v_ms, 2) + "]", "var"),
            ("^3", "const"),
        ]
    if row_key == "accel":
        return [
            ...
            ("a[" + fmt_float(getattr(state, "accel_ms2", 0.0) if state is not None else 0.0, 2) + "]", "var"),
            ("×", "const"),
            ("v[" + fmt_float(v_ms, 2) + "]", "var"),
        ]
    if row_key == "grade":
        return [
            ("m[" + fmt_float(mass, 1) + "]×g[" + fmt_float(g, 2) + "]×", "const"),
    ...
    if row_key == "engine_supply":
        return [
            ...
            ("/η[" + fmt_float(eta_d, 2) + "]:0", "const"),
        ]
    if row_key == "drivetrain_loss":
        return [
            ("ON?", "const"),
            ("Engine[" + fmt_float(engine_supply, 1) + "]", "var"),
            ("-", "const"),
            ("Wheel[" + fmt_float(wheel, 1) + "]", "var"),
            (":0", "const"),
        ]
...
def _estimate_token_width(text):
    return max(6, int(len(text) * 4.2) + 1)
